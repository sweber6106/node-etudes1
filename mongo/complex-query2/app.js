...
//
// MongoDB/mongoose app that demonstrates complex queries.
//
...
function defineSchema(callback) {
  ...
  NumberedThing = mongoose.model('NumberedThing', thingSchema);
...
function startExpress(callback) {
  ...
  app.get(apiUrl + '/things*', function(req, res) {
    var urlObj = url.parse(req.originalUrl, true, false);

    var inputQuery = urlObj.query;
    ...
    if (keys.length === 1) {
      ...
      var query = NumberedThing.find({});
   
      console.log('typeof(key):', typeof(key));
      console.log('value of key:', key);

      switch (inputQuery.category) {
        case 'good':
          query.find({ 'value' : { $gte : 200 }});
          query.find({ 'value' : { $lte : 299 }});
          break;

        case 'fair':
          query.find({ 'value' : { $in : [480, 483, 486, 487, 491] }});
          break;

        case 'poor':
          query.find({ $or: [{ value : { $lt : 200 }}, { value : { $gte : 300 }}] });
          query.find({ 'value' : { $nin : [480, 483, 486, 487, 491] }});
          break;

        default:
          body = 'Invalid SIP response category specified: ' + inputQuery[key];
