...
function defineSchema(callback) {
  ...
  TechBook = mongoose.model('TechBook', bookSchema);
...
function startExpress(callback) {
  ...
  app.get(apiUrl + '/books', function(req, res) {
    var query = TechBook.find({});
   
    query.where('copyright').gt(2008);
    //query.where('author', 'Mark Wandschneider');
    query.find({ author : 'Dr. Axel Rauschmayer' });
 
    //TechBook.find({}).exec(function(err, results) {
    query.exec(function(err, results) {
      if (!err) {
        for (var i = 0; i < results.length; ++i) {
          console.log();
          console.log('Author ......', results[i].author);
          console.log('Title .......', results[i].title);
          console.log('Publisher ...', results[i].publisher);
          console.log('Copyright ...', results[i].copyright);
          console.log();
        }
        if (results.length) {
