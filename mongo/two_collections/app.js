...
//
// MongoDB/mongoose with multiple schemas
//
...
function defineSchema(callback) {
  ...
  var doublegunSchema = mongoose.Schema({
    manufacturer:   String,
    model:          String
  });

  WildflowerRecord = mongoose.model('WildflowerRecord', wildflowerSchema);
  DoublegunRecord = mongoose.model('DoublegunRecord', doublegunSchema);
...
function startExpress(callback) {
  ...
  app.post(apiUrl + '/guns', function(req, res) {
    var gunRecord = new DoublegunRecord();

    console.log(req.body);

    gunRecord.manufacturer = req.body.manufacturer;
    gunRecord.model = req.body.model;

    gunRecord.save(function(err) {
      if (err) {
        console.log('Error in record save callback');
        ...
        res.end('gun saved in database' + '\n');
      }
    });
  });

  app.get(apiUrl + '/guns', function(req, res) {
    DoublegunRecord.find({}).exec(function(err, results) {
      if (!err) {
        for (var i = 0; i < results.length; ++i) {
          console.log();
          console.log('Manufacturer ...', results[i].manufacturer);
          console.log('Model ..........', results[i].model);
          console.log();
        }
        if (results.length) {
